...
| 版本 | 日期       | 更新内容                                                                                     |
| ---- | ---------- | -------------------------------------------------------------------------------------------- |
| v3.5 | 2026-08-26 | 文档漂移机器校验 `check:docs`（check-docs-drift.js 按本分支基线改写：禁词反转、技能数 7、browserslist 范围语义）并接入 CI；构建编排收敛（删 build-packages.sh 约 300 行，build.sh 薄壳委托 pnpm -r run build，因 rollup 不清 dist 默认保留前置清理）；customConditions 解析收敛（新增 tsconfig.base.json：moduleResolution bundler + customConditions development；shared exports 改嵌套形态；应用去 @my-app/* paths）；源码注释统一中文（HomeView 导航按钮示例、PlaygroundPage 中文化、router chunkName 复制错误修复、coding-standards 补注释约定并修正 import.meta.env → process.env.VUE_APP_*）；技能模板同步（create-vue-app / create-a-package / create-component examples）；CI actions v3→v4 + pnpm store 缓存；.browserslistrc 改 chrome >= 49 范围下限语义 |
