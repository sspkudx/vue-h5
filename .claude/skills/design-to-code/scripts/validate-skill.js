...
const VALIDATION_RULES = {
    ...
    examplesMd: {
        minExamples: 4,
        minCodeBlocks: 5,
        examplePatterns: [/^##\s+示例\s+\d+:/, /^##\s+Example\s+\d+:/i],
    },

    referenceMd: {
        requiredSections: ['TypeScript 规范', 'Vue 3 规范', 'CSS Modules 规范', 'PostCSS 配置'],
    },
};
...
function validateExamplesMd(skillDir) {
    ...
    if (codeBlockCount >= VALIDATION_RULES.examplesMd.minCodeBlocks) {
        console.log(
            `   ✅ 包含 ${codeBlockCount} 个代码块（要求至少 ${VALIDATION_RULES.examplesMd.minCodeBlocks} 个）`
        );
        results.passed++;
    } else {
        console.log(
            `   ⚠️  只有 ${codeBlockCount} 个代码块（要求至少 ${VALIDATION_RULES.examplesMd.minCodeBlocks} 个）`
        );
        results.warnings++;
