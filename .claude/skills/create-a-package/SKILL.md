...
<template>
    <div>
        ...
        <Input v-model="inputValue" placeholder="请输入内容" @blur="handleBlur" />
    </div>
</template>
...
Web 控制台（`pnpm dev`）的「＋ 新建包」可一键生成新包（模板见 `scripts/dev-launcher/templates/package/`，与本节模板基线同源）：表单支持名称/描述/类型（工具库 / 组件库 / 工具函数集 / 插件库）与「附带测试」勾选；新包首次进入 workspace 锁文件需执行 `pnpm install`（对话框内可一键后台触发）；创建成功后经实时扫描自动出现在列表，不自动启动。
